<?php
    ...
    for ($c=0;$c<count($columns);$c++) {
        $col_width = 190 * ($column_widths[$c] / 100);
        $pdf->Cell($col_width,5,iconv('UTF-8', 'windows-1252', $columns[$c]),1,0,'C',1);
    }
    $pdf->Ln(5);
    $pdf->SetFillColor(255,255,255);
    $pdf->SetX(10);
    ...
    for ($c=0;$c<count($columns);$c++) :
        ...
        if ($c == count($columns)-2) :
            $pdf->SetFont('Arial','B',9);
            $pdf->Cell($col_width,5,'Subtotal',0,0,'R',0);
    ...
    for ($c=0;$c<count($columns);$c++) :
        ...
        if ($c == count($columns)-2) :
            $pdf->SetFont('Arial','B',9);
            $pdf->Cell($col_width,5,'Discount',0,0,'R',0);
        elseif ($c == count($columns)-1) :
            $pdf->SetFont('Arial','',9);
    ...
    for ($c=0;$c<count($columns);$c++) :
        ...
        if ($c == count($columns)-2) :
            $pdf->SetFont('Arial','B',9);
            $pdf->Cell($col_width,5,"Total",0,0,'R',0);
        elseif ($c == count($columns)-1) :
            $pdf->SetFont('Arial','',9);
